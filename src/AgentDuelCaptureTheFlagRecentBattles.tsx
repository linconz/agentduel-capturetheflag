...
const MAP_THUMBNAILS: Readonly<Record<string, { path: string; width: number; height: number }>> = {
  ...
  bannerhold_heights: { path: '/resources/v1/map/thumb/bannerhold-heights-small.png', width: 180, height: 116 }
};

export function AgentDuelCaptureTheFlagRecentBattles(props: AgentDuelCaptureTheFlagRecentBattlesProps) {
  const locale = normalizeLocale(props.locale ?? 'zh-CN');
  return (
    <CaptureTheFlagI18nBoundary locale={locale} mode={props.i18nMode}>
      <CaptureTheFlagRecentBattlesContent {...props} normalizedLocale={locale} />
...
function CaptureTheFlagRecentBattlesContent({
  ...
  onUnauthorized,
  style
}: AgentDuelCaptureTheFlagRecentBattlesProps & { normalizedLocale: 'zh-CN' | 'en-US' }) {
  ...
  const [records, setRecords] = useState<CaptureTheFlagBattleRecordsPage | null>(null);
  const [filters, setFilters] = useState<CaptureTheFlagBattleRecordFilters>(emptyCaptureTheFlagBattleRecordFilters);
  ...
  const loadContext = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      setOwnedPublicIds(new Set(context.ownedTeamPublicIds));
  ...
  const rows = useMemo(
    () => filterCaptureTheFlagBattleRecordRows(
      createCaptureTheFlagBattleRecordRows(records?.battles ?? [], ownedPublicIds, {
        getTeamHref,
        getReplayHref,
        getRevengeHref
      }),
    ...
    [filters, getTeamHref, getReplayHref, getRevengeHref, ownedPublicIds, records]
  );
  ...
  const rootClassName = ['agentduel-capturetheflag', 'capturetheflag-recent-battles', className ?? ''].filter(Boolean).join(' ');
  return (
    <div className={rootClassName} style={style}>
      ...
      {contextStatus === 'ready' ? (
        <BattleRecordsView
          ...
          onClearFilters={() => setFilters(emptyCaptureTheFlagBattleRecordFilters)}
...
function BattleRecordsView({
  ...
  onClearFilters(): void;
  onLoadMore(): void;
  onSetFilters(filters: CaptureTheFlagBattleRecordFilters): void;
  records: CaptureTheFlagBattleRecordsPage | null;
  recordsError: string | null;
  recordsStatus: RecordsStatus;
  rows: CaptureTheFlagBattleRecordRow[];
}) {
  const { t } = useTranslation();
  ...
  return (
    <>
      ...
      <section className="battle-records-hero" aria-labelledby="battle-records-title">
        ...
        <p>{t('dashboard.records.fixedModeCopy', { mode: t('dashboard.mode.captureTheFlag') })}</p>
      </section>
      <section className="dashboard-section" aria-labelledby="battle-record-list-title">
        <div className="dashboard-section-heading">
          ...
          <div className="battle-record-filter-menu-shell">
            <Button
              aria-controls="captureTheFlag-battle-record-filter-menu"
            ...
            {isFilterMenuOpen ? (
              <div className="battle-record-filter-menu" id="captureTheFlag-battle-record-filter-menu" role="dialog" aria-label={t('dashboard.records.filterMenuAria')}>
...
function BattleRecordRow({
  ...
  linkComponent?: CaptureTheFlagLinkComponent;
  onAddFilter(option: BattleRecordFilterOption): void;
  row: CaptureTheFlagBattleRecordRow;
}) {
  const { t } = useTranslation();
...
function BattleRecordTitle({ row, linkComponent: Link }: { row: CaptureTheFlagBattleRecordRow; linkComponent?: CaptureTheFlagLinkComponent }) {
  const { t } = useTranslation();
...
function renderParticipantName(name: string, href: string | null, own: boolean, Link?: CaptureTheFlagLinkComponent) {
...
function BattleDateMapMeta({ assetBaseUrl, dateFormatter, row }: { assetBaseUrl: string; dateFormatter: Intl.DateTimeFormat; row: CaptureTheFlagBattleRecordRow }) {
  const { t } = useTranslation();
    ...
    <p className="battle-date-map-meta">
      ...
      <span className="battle-map-label" tabIndex={0}>
        ...
        <span className="battle-map-tooltip" role="tooltip">
          ...
          <span>{t(`battleMap.descriptions.captureTheFlag.${row.mapId}`, { defaultValue: t('battleMap.previewUnavailable') })}</span>
        </span>
      </span>
    </p>
  );
...
function LoadMore({ onLoadMore, records, recordsError, recordsStatus }: { onLoadMore(): void; records: CaptureTheFlagBattleRecordsPage | null; recordsError: string | null; recordsStatus: RecordsStatus }) {
  const { t } = useTranslation();
...
function hasFilterOption(filters: CaptureTheFlagBattleRecordFilters, option: BattleRecordFilterOption): boolean {
  switch (option.kind) {
