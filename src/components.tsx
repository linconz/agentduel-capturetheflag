...
import type { CaptureTheFlagLinkComponent, CaptureTheFlagLinkProps } from './types';

export function DefaultLink({ children, ...props }: CaptureTheFlagLinkProps) {
  ...
  return (
    <button
      ...
      className={[
        'duel-button',
        `duel-button-${variant}`,
        `duel-button-tone-${tone}`,
        `duel-button-size-${size}`,
        `duel-button-width-${width}`,
        loading ? 'is-loading' : '',
        className ?? ''
      ].filter(Boolean).join(' ')}
